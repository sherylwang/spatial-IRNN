...
namespace caffe {
// count(total number), bottom_data(enter data), foward(forward pass:true, backward pass: false)
// permute_order: pointer to blob data, （axis,1,1,1）,each value in the first dimension stores the value
// old_steps, new_steps: pointer to blob data, (axis,1,1,1),
//each value in the first dimension stores the value
...
void PermuteLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  num_axes_ = bottom[0]->num_axes(); //number of bottom blob dimensions
  vector<int> orders;
  ...
  for (int i = 0; i < num_axes_; ++i) {
    if (std::find(orders.begin(), orders.end(), i) == orders.end()) {//didn't find, put it into the blob
      orders.push_back(i);
    }
  }
  ...
  for (int i = 0; i < num_axes_; ++i) {
    if (orders[i] != i) {
      ...
      need_permute_ = true; //hahaha, rigorous code  
      break;
    }
  }
...
void PermuteLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num_axes_; ++i) {
    ...
    //is it redundant? same as the operation at the end of layersetup
